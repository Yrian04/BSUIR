...
class CurveEditor:
    ...
    def new_curve(self):
        ...
        if CurveClass := self.curve_classes.get(curve_type_name):
            curve = CurveClass()
            self.curves.append(curve)
            self._extracted_from_clear_all_8(curve)
        else:
    ...
    def clear_all(self):
        ...
        if messagebox.askyesno("Подтверждение", "Вы уверены, что хотите удалить все кривые?"):
            self.curves.clear()
            self._extracted_from_clear_all_8(None)

    # TODO Rename this here and in `new_curve` and `clear_all`
    def _extracted_from_clear_all_8(self, arg0):
        self.active_curve = arg0
        self.selected_curve = None
        self.redraw()
    ...
    def on_click(self, event):
        for curve in reversed(self.curves):
            for i, point in enumerate(curve.points):
                if abs(event.x - point.x) <= 5 and abs(event.y - point.y) <= 5:
                    return self._extracted_from_on_click_5(curve, i, event)
        if self.active_curve:
            add_x, add_y = event.x, event.y
            if self.snap_enabled.get():
                if nearest_point := self.find_nearest_point(
                    event.x, event.y, exclude_curve=self.active_curve
                ):
    ...
    # TODO Rename this here and in `on_click`
    def _extracted_from_on_click_5(self, curve, i, event):
    ...
    def on_drag(self, event):
        if self.selected_curve and self.drag_data["point_idx"] != -1:
            ...
            if self.snap_enabled.get():
                if nearest_point := self.find_nearest_point(
                    event.x, event.y, exclude_curve=self.selected_curve
                ):
