...
def remove_message(message_id: int):
    try:
        response = requests.post(f"http://localhost:8000/remove_message/{message_id}")
        if response.status_code == 200:
            return True
        else:
            st.error(f"Failed to remove message. Error: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        st.error(f"An error occurred while removeing the message: {str(e)}")
        return None
