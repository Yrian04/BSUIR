    ...
    public class UndirectedGraph<T> : ICollection<T>, ICollection, ICloneable
    {
        ...
        public bool this[int index1, int index2]
        {
            get
            {
                ...
                return IncidenceMatrix[index1, index2];
            }
            ...
            set
            {
                ...
                IncidenceMatrix[index1, index2] = value;
            }
        }
        ...
        public void Add(T item)
        {
            ...
            if (IncidenceMatrix is not null)
            {
            ...
            {
                IncidenceMatrix = new bool[Vertexes.Count, Vertexes.Count];
            }
        ...
        public bool Remove(T item)
        {
            ...
            if (success)
            {
                ...
                for (int i = 0; i < IncidenceMatrix!.GetLength(0); i++)
                    for (int j = 0; j < IncidenceMatrix!.GetLength(1); j++)
                        if (i != index || j != index)
                            newMatrix[i > index ? i - 1 : i, j > index ? j - 1 : j] = IncidenceMatrix![i, j];
        ...
        public object Clone()
        {
            ...
            bool[,]? incidentMatrix = (bool[,]?)IncidenceMatrix?.Clone();
        ...
        public override string ToString()
        {
            ...
            for (var i = 0; i < IncidenceMatrix.GetLength(0); i++)
            {
                ...
                for (var j = 0; j < IncidenceMatrix.GetLength(1); j++)
                    output += "\t" + (IncidenceMatrix[i, j] ? '1' : '0');
            }
        ...
        public override bool Equals(object? obj)
        {
            if (obj is UndirectedGraph<T> graph)
                return Vertexes.SequenceEqual(graph.Vertexes) && IncidenceMatrix == graph.IncidenceMatrix;
